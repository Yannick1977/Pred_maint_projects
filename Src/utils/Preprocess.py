...
class utils_preprocess:
    def __init__(self, df_: pd.DataFrame, cfg_path: ConfigBox):
        """
        Initializes an instance of the utils_preprocess class.
        """
        self.df_ = df_
    ...
    def preprocess(self, chx_target:bool, chx_smote:bool)->tuple:
        """
        ...
        self.df_ = self.df_[self.df_['Failure Type'] != 'Random Failures']
        ...
        num_cols = self.df_.select_dtypes(exclude='O').columns.to_list()
        num_cols.remove('Target')

        if chx_target:
            # Drop the 'Target' column and rename the 'Failure Type' column to 'Target'
            self.df_.drop(columns=['Target'], inplace=True)
            self.df_.rename(columns={'Failure Type': 'Target'}, inplace=True)
        else:
            # Drop the 'Failure Type' column
            self.df_.drop(columns=['Failure Type'], inplace=True)
        ...
        if chx_target:
            ...
            self.df_['Target'] = self.df_['Target'].replace({1: 'With_Failure', 0: 'No Failure'})
            ct_y_ = ColumnTransformer(
                transformers=[
                    ("fail", OrdinalEncoder(), ['Target'])
                ],
        ...
        X_, X_val, y_, y_val = train_test_split(
            self.df_.drop(columns=['Target']), 
            self.df_['Target'], 
            test_size=0.2,
            stratify=self.df_['Target'],
            random_state=42)
        ...
        if chx_smote:
            X_train_res_, y_train_res_ = self.apply_smote(X_=X_train_trans, y_=y_train_trans)
            X_test_res_, y_test_res_ = self.apply_smote(X_=X_test_trans, y_=y_test_trans)
        else:
        ...
        df_train = self.create_df_from_X_y(X_=X_train_res_, y_=y_train_res_, columns_y=dict_target_inv_.values())
        df_train.to_csv(self.cfg_path.local_data_file_train, index=False)
        df_test = self.create_df_from_X_y(X_=X_test_res_, y_=y_test_res_, columns_y=dict_target_inv_.values())
        df_test.to_csv(self.cfg_path.local_data_file_test, index=False)

        return df_train, df_test, dict_target_inv_

    def fit_transform(self, ct_, train, test, val):
    ...
    def create_df_from_X_y(self, X_, y_, columns_y):
