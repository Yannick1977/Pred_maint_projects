...
class modelMaintenance :
    ...
    def __init__(self, num_layers, num_input, num_output, loss, optimizer, metrics, path_model: str) -> None:
            """
    ...
    def _init_model(self, num_layers, num_input, num_output, loss, optimizer, metrics, activation='relu', dropout=0.2):
        ...
        for n in lst_num_neurons:
            ...
            #model.add(Dense(n, activation=activation, kernel_regularizer=l2(0.01)))
    ...
    def plot_model(self):
        ...
        visualizer(self.model, file_name=self.path_model+'/model', file_format='png', view=False)
        print(self.path_model+'/model.png')
        img = mpimg.imread(self.path_model+'/model.png')
    ...
    def predict_proba(self, X):
        ...
        Returns:
            array-like: Predicted output probabilities.
        """
        return self.model.predict_proba(X)
    
    def load_model(self, path_model: str):
        """
        Loads the weights of the model from the specified path.

        Parameters:
        - path_model (str): The path to the model weights file.
    ...
    def save_model(self, path_model: str):
        """
        Saves the weights of the model to the specified path.
    ...
    def _create_callbacks(self):
            ...
            self.early_stopping = EarlyStopping(monitor=metric, patience=5, restore_best_weights=True, min_delta=0.001)
            
            self.reduce_lr = ReduceLROnPlateau(monitor=metric, factor=0.2, patience=5, min_lr=1e-5)
            
            return [self.ModelCheckpoint, self.early_stopping, self.reduce_lr]
            #return [self.early_stopping, self.reduce_lr]
